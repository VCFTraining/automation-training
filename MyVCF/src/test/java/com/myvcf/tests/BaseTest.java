...
@org.testng.annotations.Listeners({ com.myvcf.listeners.ExtentTestListener.class,
...
public abstract class BaseTest {
	...
	/**
	 * STEP counter (Thread-safe, resets automatically on test start + manually per
	 * data record)
	 */
	...
	public void setupDriver(@Optional("chrome") String browser, @Optional("false") String headless,
			@Optional("10") String waitSecondsParam) {

		STEP_NO.set(1); // ensure fresh STEP numbers each test start

		final String resolvedBrowser = (browser == null || browser.isBlank()) ? ConfigReader.get("browser", "chrome")
				: browser;

		final boolean resolvedHeadless = (headless == null || headless.isBlank())
		...
		System.out.println(
				"[BaseTest] Starting WebDriver -> browser=" + resolvedBrowser + ", headless=" + resolvedHeadless);
		...
		// Resolve default base URL from config so the run knows what environment is
		// being used
	...
	protected void step(String message) {
		...
		if (t != null)
			t.log(com.aventstack.extentreports.Status.INFO, line);
		STEP_NO.set(n + 1);
	}
	...
	protected void passStep(String message) {
		...
		var t = com.myvcf.reporting.ExtentReportManager.getTest();
		if (t != null)
			t.log(com.aventstack.extentreports.Status.PASS, line);
		STEP_NO.set(n + 1);
	}
	...
	// Captures a screenshot as Base64 and embeds it directly into the Extent
	// report.
		...
		try {
			...
			if (base64 != null) {
				String cap = (caption == null || caption.isBlank())
						...
						: caption;
				t.info(cap, com.aventstack.extentreports.MediaEntityBuilder
						.createScreenCaptureFromBase64String(base64, cap).build());
			}
		} catch (Exception e) {
	...
	protected void stepInfo(String message) {
		...
		var t = com.myvcf.reporting.ExtentReportManager.getTest();
		if (t != null)
			t.log(com.aventstack.extentreports.Status.INFO, message);
	}
	...
	protected File captureScreenshot(String nameHint) {
		...
		try {
			...
			if (!(snapDriver instanceof TakesScreenshot)) {
				try {
					snapDriver = new org.openqa.selenium.remote.Augmenter().augment(driver);
				} catch (Exception ignored) {
				}
			}
			if (!(snapDriver instanceof TakesScreenshot))
	...
	// Opens the default base URL only if the browser is still on a blank/about/data
	// page.
	protected void navigateToBaseUrl() {
		try {
			...
			boolean looksBlank = (current == null || current.isBlank() || current.startsWith("data:")
	...
	protected void openUrl(String url) {
		if (driver == null)
			throw new IllegalStateException("[BaseTest] Driver not initialized.");
		driver.get(url);
