...
public abstract class BasePage {
	...
	protected static String digitsOnly(String s) {
		return s == null ? "" : s.replaceAll("\\D+", "");
	}

	// Escapes single quotes so the value is safer to use inside XPath or JS
	// strings.
	...
	// This helps when normal Selenium click is flaky because of overlays, stale
	// elements, or scrolling issues.
	protected void safeClick(By locator) {
		...
		for (int i = 1; i <= attempts; i++) {
			try {
				...
				last = new RuntimeException("safeClick(By) attempt " + i + " failed: " + e.getClass().getSimpleName()
						+ " - " + e.getMessage(), e);
				try {
	...
	// Same safe click idea, but this version works with an already found
	// WebElement.
	...
	// Tries different locator patterns to find an input that belongs to the given
	// label.
	protected WebElement tryFindInputByLabels(String... labels) {
		for (String label : labels) {
			...
			for (By by : List.of(By.xpath(
					"//input[contains(translate(@aria-label,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'), '"
							+ escape(lower) + "')][not(@type='radio')][not(@type='checkbox')]"),
					By.xpath(
							"//input[contains(translate(@placeholder,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'), '"
									+ escape(lower) + "')][not(@type='radio')][not(@type='checkbox')]"))) {
				for (WebElement c : driver.findElements(by))
					if (c.isDisplayed())
						return c;
			}

			List<WebElement> grouped = driver.findElements(By.xpath("//*[contains(@class,'slds-form-element')]"
	...
	protected WebElement attributeShortcut(String lowerLabel) {
		...
		if (css == null)
			return null;
		for (WebElement c : driver.findElements(By.cssSelector(css)))
			if (c.isDisplayed())
				return c;
		return null;
	}
	...
	protected void clearAndType(WebElement el, String text) {
		...
		try {
			explicitWait().until(ExpectedConditions.elementToBeClickable(el));
		} catch (Exception ignored) {
		}
		try {
			el.click();
	...
	protected void selectByVisibleTextNearLabel(String label, String value) {
		...
		// SLDS combobox
		List<WebElement> triggers = container
				.findElements(By.xpath(".//*[@role='combobox'] | .//input[contains(@class,'slds-combobox__input')]"
		...
		for (WebElement t : triggers)
			if (t.isDisplayed()) {
				...
				safeClick(t);
				try {
					t.sendKeys(Keys.chord(Keys.CONTROL, "a"));
	...
	// Highlights the element in the browser so it is easier to see during
	// debug/demo runs.
		...
		try {
			((JavascriptExecutor) driver).executeScript(
					"const e=arguments[0];e.setAttribute('data-prev-style',e.getAttribute('style')||'');"
							+ "e.setAttribute('style',(e.getAttribute('style')||'')+';outline:3px solid orange; outline-offset:2px;');",
					el);
		} catch (Exception ignored) {
		}
...
//        System.out.println(msg);
	...
	// Clicks a radio option by first finding the group, then the matching option
	// inside it.
	...
	protected void clickRadio(String groupLabel, String optionText) {
		...
		if (container != null && tryClickOptionInside(container, o))
			return;
		if (tryClickOptionInside(driver.findElement(By.tagName("body")), o))
			return;
		...
		for (By by : List.of(By.xpath(
				"//input[@type='radio' and translate(@value,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz')='"
						+ escape(o) + "']"),
				By.xpath(
						"//input[@type='radio' and @value and contains(translate(@value,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'"
								+ escape(o) + "')]"))) {
			List<WebElement> radios = driver.findElements(by);
			for (WebElement r : radios) {
				if (r.isDisplayed()) {
					highlight(r, "Radio input (value=" + optionText + ")");
					safeClick(r);
					return;
				}
			}
	...
	private boolean tryClickOptionInside(WebElement scope, String lowerOption) {
		...
		for (WebElement lab : labels)
			if (lab.isDisplayed()) {
				highlight(lab, "Radio label: " + lab.getText());
				safeClick(lab);
				return true;
			}
		...
		List<WebElement> spanLabels = scope.findElements(By.xpath(
				".//label[.//span[contains(translate(normalize-space(.),'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'), '"
						+ escape(lowerOption) + "')]]"));
		for (WebElement lab : spanLabels)
			if (lab.isDisplayed()) {
				highlight(lab, "Radio label(span): " + lab.getText());
				safeClick(lab);
				return true;
			}
		...
		List<WebElement> roleRadios = scope.findElements(By.xpath(
				".//*[@role='radio'][contains(translate(@aria-label,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'), '"
						+ escape(lowerOption) + "') "
						+ " or contains(translate(normalize-space(.),'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'), '"
						+ escape(lowerOption) + "')]"));
		for (WebElement r : roleRadios)
			if (r.isDisplayed()) {
				highlight(r, "Role=radio option");
				safeClick(r);
				return true;
			}
		...
		for (WebElement t : texts) {
			List<WebElement> radios = t.findElements(
					By.xpath(".//preceding::input[@type='radio'][1] | .//following::input[@type='radio'][1]"));
			for (WebElement r : radios)
				if (r.isDisplayed()) {
					highlight(r, "Radio input near text");
					safeClick(r);
					return true;
				}
		}
	...
	/**
	 * Type digits into masked/segmented SLDS/LWC inputs found by label. - Focuses
	 * the visible input. - Distributes digits across sibling segments when present
	 * (maxlength). - Types real key events via Actions. - Falls back to JS set +
	 * input/change events if the mask did not stick.
	 */
	...
	protected void enterMaskedDigitsByLabel(String digits, String... labels) {
		...
		// Try to collect all visible inputs within the same form element (handles split
		// masks like 3-2-4)
		List<WebElement> inputs = first.findElements(By.xpath("./ancestor::*[contains(@class,'slds-form-element')][1]"
				+ "//input[not(@type='hidden') and not(@type='radio') and not(@type='checkbox')]"));
		inputs.removeIf(el -> !el.isDisplayed());

		if (inputs.isEmpty())
			inputs = List.of(first); // single field case
		...
		for (WebElement in : inputs) {
			...
			try {
				...
				if (ml != null && !ml.isBlank())
					max = Integer.parseInt(ml.trim());
			} catch (Exception ignored) {
			}
			if (max <= 0)
				max = d.length() - idx; // no maxlength → dump the rest
	...
	private void focusAndTypeRealKeys(WebElement el, String text) {
		try {
			new Actions(driver).moveToElement(el).click().keyDown(Keys.CONTROL).sendKeys("a").keyUp(Keys.CONTROL)
					.sendKeys(Keys.DELETE).pause(Duration.ofMillis(Math.max(20, (int) keyDelay))).sendKeys(text)
					.pause(Duration.ofMillis(Math.max(20, (int) keyDelay))).sendKeys(Keys.TAB).build().perform();
		} catch (Exception e) {
	 ...
	 */
	// Last-resort fallback that sets the value with JavaScript and fires
	// input/change events.
	private void jsSetWithEvents(WebElement el, String digits) {
		((JavascriptExecutor) driver).executeScript("const e=arguments[0], v=arguments[1];" + "e.value=v;"
				+ "e.dispatchEvent(new Event('input',{bubbles:true}));"
				+ "e.dispatchEvent(new Event('change',{bubbles:true}));", el, digits);
	}

	// Verifies that the masked field kept the expected digits after typing and
	// blur.
